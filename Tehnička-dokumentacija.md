...
## Use-Case Dijagram
Za aplikaciju smo kreirali jedan use case dijagram. Na dijagramu možemo vidjeti tri učesnika, a to su organizator turnira, korisnici tj. osobe koje se prijavljuju na turnir i vanjski servis registrationmenu koji dohvaća upitnik za upis podataka unutar aplikacije. Organizator vrši CRUD operacije nad registriranim korisnicima. Organizator nasljeđuje sve karakteristike korisnika. Korisnik nakon registracije pristupa početnoj formi odakle pristupa svim ostalim pregledima koji su prikazani unutar aplikacije. Kod pregleda timova svaki korisnik može pregledati i sve članove koji se nalaze unutar prijavljenog tima. U dijelu statistike možete si ispisati statistiku i skinuti na vaše računalo.
![use case dijagram](https://github.com/foivz/r20--aoletic-tmusic-dabramov1/blob/master/Dijagrami/use%20case%20diagram2.png) 
## Dijagram klasa
...
![Dijagram klasa](https://github.com/foivz/r20--aoletic-tmusic-dabramov1/blob/master/Dijagrami/SportsPlannerAppClassDiagramFinalni.png)
## ERA model
Baza podataka implementirana je u MySql-u te je pohranjena na serveru kolegija Programsko inženjerstvo. Model baze podataka sastoji se od 6 tablica. Tablica "Korisnik" sadrži informacije o bilo kojem korisniku naše aplikacije. Tablica "Korisnik" vuče podatke o tome koju ulogu određeni korisnik ima. U našoj aplikaciji to su administrator, organizator događaja te natjecatelj. Tablica "Dogadaji" sadrži podatke o sportskim događajima koje su organizatori događaja kreirali. Događaj mora biti određene vrste sporta koje je administrator zadao u tablici "Sportovi". U tablici "Timovi" su podatci o timovima koje su natjecatelji prijavili za određeno natjecanje. U tablici "Natjecatelji" nalaze se svi natjecatelji s ID-jem njihovog tima, a ukoliko su to pojedinačna natjecanja onda tima nema. U tablici "Utakmice" nalaze se sve utakmice generirane natjecateljskim stablom. Ovdje su ujedno dva atributa koja nisu obavezna kod kreiranja utakmica, a kasnije se popunjavaju kod upisa rezultata. Korisnik može kreirati više timova, dok jedan tim može biti kreiran od samo jednog korisnika. Isto tako vrijedi i za tablicu "Natjecatelji", "Dogadaji", "Sportovi" te "Utakmice". Tim može imati više natjecatelja, dok jedan natjecatelj može pripadati samo jednom timu.
![](https://github.com/foivz/r20--aoletic-tmusic-dabramov1/blob/master/Dijagrami/ERASportsEventPlanner.png)
